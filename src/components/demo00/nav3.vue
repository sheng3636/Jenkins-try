...
<!-- VUE饿了么树形控件添加增删改功能按钮 -->
<script>
import TreeRender from '@/components/common/nx-tree'
import api from '../../api/treeApi.js'
export default{
  name: 'tree',
  data() {
    return {
  ...
  methods: {
      allowDrop(draggingNode, dropNode, type) {  //拖拽时判定目标节点能否被放置
        if(draggingNode.data.pid==1&&dropNode.data.pid==1){
            ...
            if(draggingNode.data.pid==2&&dropNode.data.pid==2){
                if(draggingNode.parent.id == dropNode.parent.id){
